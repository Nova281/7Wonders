...
public class GamePanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		if (gs.ifEnd()) {
			...
			if(gs.getCurrentPlayer().getWonder().getPhaseState(1))
				try {
					g.drawImage(ImageIO.read(getClass().getResource("/tokens/check.png")), 480, 680, 230, 80, this);
			...
			if(gs.getCurrentPlayer().getWonder().getPhaseState(2))
				try {
					g.drawImage(ImageIO.read(getClass().getResource("/tokens/check.png")), 840, 680, 230, 80, this);
				} catch (IOException e2) {
					// TODO Auto-generated catch block
					e2.printStackTrace();
				}
			if(gs.getCurrentPlayer().getWonder().getPhaseState(3))
				try {
					g.drawImage(ImageIO.read(getClass().getResource("/tokens/check.png")), 1190, 680, 230, 80, this);
				} catch (IOException e2) {
					// TODO Auto-generated catch block
					e2.printStackTrace();
				}
	...
	public void updateGameState(GameState gs) throws IOException {
		this.gs = gs;
		LinkedHashMap<Integer, Boolean> phaseMap = new LinkedHashMap<>();
		for(int i = 1; i < 4; i++) {
			phaseMap.put(i, gs.getCurrentPlayer().getWonder().getPhaseState(i));
		}
